...
export const ModuleCalendar = () => {
    ...
    const date = Variable('', {
        poll: [
            ...
            () => GLib.DateTime.new_now_local().format(userOptions.asyncGet().time.calendarDateFormat),
        ],
    })
    ...
    const mainStack = Widget.Stack({
        ...
        transitionDuration: userOptions.asyncGet().animations.durationLarge,
        setup: (stack) => Utils.timeout(1, () => {
            stack.shown = userOptions.asyncGet().sidebar.calendar.expandByDefault ? 'expanded' : 'collapsed';
        })
    })
