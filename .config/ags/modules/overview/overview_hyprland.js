...
export default () => {
    ...
    const Workspace = (index) => {
        ...
        const WorkspaceNumber = ({ index, ...rest }) => Widget.Label({
            ...
            label: `${index}`,
            css: overviewMonitor.bind().as(monitor => `
                margin: ${Math.min(monitors[monitor].width, monitors[monitor].height) * userOptions.asyncGet().overview.scale * userOptions.asyncGet().overview.wsNumMarginScale}px;
                font-size: ${monitors[monitor].height * userOptions.asyncGet().overview.scale * userOptions.asyncGet().overview.wsNumScale}px;
            `),
        ...
        const widget = Widget.Box({
            ...
            css: overviewMonitor.bind().as(monitor => `
                min-width: ${1 + Math.round(monitors[monitor].width * userOptions.asyncGet().overview.scale)}px;
                min-height: ${1 + Math.round(monitors[monitor].height * userOptions.asyncGet().overview.scale)}px;
            `),
